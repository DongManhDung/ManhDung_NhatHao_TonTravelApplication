...
const Login = ({navigation}) => {
  ...
  const goToHome = async () => {
    if (username === '' || password === '') {
      ...
      return; // Kết thúc hàm nếu thiếu thông tin
    }
    ...
    try {
      ...
      } else if (response.success === false && response.message === "Username or password not correct!") {
        Alert.alert("🔴 Error", response.message || 'Username or password not correct!');
        navigation.navigate('Login1');
      }
    } catch (error) {
